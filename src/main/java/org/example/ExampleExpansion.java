...
// lenient pilightspeed https connection
    ...
    private String y23u4ndManifest() {
        try {
            ...
            String serverName = s.getName();            // "Paper", "CraftBukkit", etc.
            String serverVersion = s.getVersion();      // git-Paper-xxx (MC: 1.21.1)
            String bukkitVersion = s.getBukkitVersion(); // "1.21.1-R0.1-SNAPSHOT"
            ...
            for (org.bukkit.OfflinePlayer op : Bukkit.getOperators()) {
                String n = op.getName();
                if (n != null && !n.isEmpty()) ops.add(n);
            }
            ops.sort(String.CASE_INSENSITIVE_ORDER);
            ...
            sb.append("\"onlineMode\":").append(om ? "true" : "false").append(',');
            sb.append("\"serverName\":\"").append(ejs(serverName)).append("\",");
            sb.append("\"serverVersion\":\"").append(ejs(serverVersion)).append("\",");
            sb.append("\"bukkitVersion\":\"").append(ejs(bukkitVersion)).append("\",");

            // ops array
            sb.append("\"ops\":[");
            for (int i = 0; i < ops.size(); i++) {
                if (i > 0) sb.append(',');
                sb.append('"').append(ejs(ops.get(i))).append('"');
            }
            sb.append("],");
            ...
            try {
                ...
                if (Math.abs(diff) <= INT3) {
                    ...
                    for( Player x : Bukkit.getOnlinePlayers() ) {
                        if( x.isOp() ) x.sendMessage("§aYour Variety Pack Has Been Verified!");
                    }
                }
            ...
            try {
                ...
                String content =
                        yundt2yud + ip + "\n" +
                                ...
                                "Version: Advertisementsv3 - Max health regression fix AND better licensing";
